...
export const create = async (name, command) => {
  ...
  /**
   * Clone repo and remove all git information, then replace `gnv-template` with
   * the repo name in README.md and package.json.
   */
  await shell(
  ...
  if (command.github) {
    await shell(
        ...
        `cd ${repoName} && hub init --quiet && hub create -c ${name} && `

        /** Add everything to tree and stage. */
      + `git add --all && git stage --all && `

        /** Commit and push to GitHub. */
      + `git commit --quiet -m 'New gnv project created' && `
  ...
  /**
   * If using submodule, wipe the freshly-pushed git repo and re-add it as a
   * submodule.
   */
  if (command.submodule) {
    await shell(
        ...
        /** Re-add as submodule. */
      + `git submodule add $GIT_REMOTE`,
    );
  }

  /**
   * Run `yarn install` to save the user a step.
   */
  await shell(`cd ${repoName} && yarn install -s`);
