...
/**
 * Install global `gnv` peer dependencies.
 */
export const installPeerDeps = async () => {
  const originalJson = readPackageJson(true);
  const peerDeps = originalJson.peerDependencies || {};
  const packageStrings = Object.entries(peerDeps).map(
  ...
  await shell(
      `npm i -g -f --no-save --silent ${packageStrings.join(' ')}`,
  );

  console.log(`Installed ${packageStrings.length} packages.`);
};
