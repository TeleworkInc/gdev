...
export const publish = async (level = 'patch') => {
  const getPackageJson = () => JSON.parse(fs.readFileSync(jsonLocation));

  /**
   * The original value of package.json.
   */
  const jsonLocation = path.resolve(process.cwd(), 'package.json');
  const originalJson = getPackageJson();
  ...
  fs.writeFileSync(
      jsonLocation,
      JSON.stringify(updatedOriginal, null, 2),
  );
};
