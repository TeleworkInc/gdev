...
// import rollupEsDist from '../.gnv/config.rollup.esDist.mjs';
...
/**
 * Run the Rollup dev pass.
 */
export const rollupDevPass = async () => {
  await Promise.all(rollupDevEs.map(
      async (config) => {
...
// import configWebpack from '../.gnv/config.webpack.cjs';
/**
 * Run the Webpack dev pass.
 */
export const webpackDevPass = async () => {
...
export const build = async (command) => {
  ...
  // await Promise.all(rollupDefault.map(
  //     async (config) => {
  //       const bundle = await rollup(config);
  //       await bundle.write(config.output);
  //     },
