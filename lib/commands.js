...
export const publish = async (level = 'patch') => {
  ...
  try {
    try {
    /**
     * Bump version, do not bother with git commit (will throw if working
     * directory not clean).
     */
      await shell(`npm version --no-git-tag-version ${level}`);
    }
    catch (e) {
      /**
       ...
       */
      console.error(
          `\n\nError bumping version with NPM. Please stage any git changes ` +
          `you've made!`,
      );
      throw new Error(e);
    }
     ...
     */
    const modifiedJson = JSON.parse(JSON.stringify(readPackageJson()));
...
export const getPeerDeps = async (pkgs) => {
  if (!pkgs.length) pkgs = [ 'gnv' ];

  useVerboseShell();
  for (const pkg of pkgs)
    await shell(
        `npx install-peerdeps -oS ${pkg}`,
